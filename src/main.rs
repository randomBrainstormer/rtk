...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Sbt { command } => match command {
            SbtCommands::Test { args } => {
                sbt_cmd::run_test(&args, cli.verbose)?;
            }
            SbtCommands::Compile { args } => {
                sbt_cmd::run_compile(&args, cli.verbose)?;
            }
            SbtCommands::Run { args } => {
                sbt_cmd::run_run(&args, cli.verbose)?;
            }
            SbtCommands::Other(args) => {
                sbt_cmd::run_other(&args, cli.verbose)?;
            }
        },
